...
public class HoldingTabFragment extends GreekBaseFragment {
    ...
    private List<DematHolding> TempdematLtpDetails;
    ...
    private  Boolean symboleAssending=false;
    private  Boolean hValueAssending=false;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        TempdematLtpDetails = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        symbol_nametxt.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                  if(!symboleAssending){
                     symboleAssending=true;
                 }else{
                     symboleAssending=false;
                 }

                if(symboleAssending){
                    Collections.sort(dematLtpDetails, new Comparator<DematHolding>() {

                        public int compare(DematHolding c1, DematHolding c2) {
                            return c1.getScripName().compareTo(c2.getScripName());
                        }
                    });
                }else{
                    Collections.sort(dematLtpDetails, new Comparator<DematHolding>() {
                    ...
                    Collections.reverse(dematLtpDetails);
                }

                commonAdapter = new MyListAdapter(dematLtpDetails, getMainActivity());
                recyclerView.setAdapter(commonAdapter);
                for (int i =0;i<dematLtpDetails.size();i++){

                    DematHolding dematHolding= dematLtpDetails.get(i);

                    if (dematHolding.getNSEToken().equalsIgnoreCase("0")) {
                        streamingList.add(dematHolding.getBSEToken());
                        commonAdapter.addSymbol(dematHolding.getBSEToken());
                    } else {
                        streamingList.add(dematHolding.getNSEToken());
                        commonAdapter.addSymbol(dematHolding.getNSEToken());
                    }
                }
            }
        });
        valuetxt.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if(!hValueAssending){
                    hValueAssending=true;
                }else{
                    hValueAssending=false;
                }

                if(hValueAssending){
                    Collections.sort(dematLtpDetails, new Comparator<DematHolding>() {
                        public int compare(DematHolding c1, DematHolding c2) {
                            return  (int)(Double.valueOf(c1.getHoldingvalue()) - Double.valueOf(c2.getHoldingvalue()));
        ...
        if (stockArrayList != null && stockArrayList.size() > 0) {
            ...
            if (islast == 1 || (stockArrayList.size() == noofrecords && islast == 2)) {
                ...
                if (noofrecords > 0) {
                    ...
                    if (String.valueOf(totalCurrentMTOM).startsWith("-")) {
                        currentMtmtxt.setTextColor(getResources().getColor(textColorNegative));

                    } else {
                        currentMtmtxt.setTextColor(getResources().getColor(textColorPositive));
                    }
...
//            BroadcastPNL = Double.parseDouble(tempCurrentMtmtxt);
    ...
    public class MyListAdapter extends RecyclerView.Adapter<HoldingTabFragment.MyListAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(HoldingTabFragment.MyListAdapter.ViewHolder holder, int position) {
            ...
            if (dematHolding.getCurrentMTM().startsWith("-")) {
