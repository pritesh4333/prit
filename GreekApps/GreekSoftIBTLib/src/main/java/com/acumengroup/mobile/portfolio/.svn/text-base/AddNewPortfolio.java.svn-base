...
public class AddNewPortfolio extends GreekBaseFragment {
                ...
                if (!restrictedGrp.contains(symbolName.getText().toString().toLowerCase().trim().toUpperCase())) {
                    ...
                    if (!grpName.contains(symbolName.getText().toString().toLowerCase().trim())) {
                        ...
                        if (GreekBaseActivity.USER_TYPE == GreekBaseActivity.USER.OPENUSER) {
                            groupAddURL = "createGroupForWatchlist?clientCode=" + AccountDetails.getDeviceID(getMainActivity()) + "&gscid=" + AccountDetails.getUsername(getMainActivity()) + "&groupName=" + symbolName.getText().toString().trim() + "&watchlistType=user";
                        } else {
                            groupAddURL = "createGroupForWatchlist?clientCode=" + AccountDetails.getClientCode(getMainActivity()) + "&gscid=" + AccountDetails.getUsername(getMainActivity()) + "&groupName=" + symbolName.getText().toString().trim() + "&watchlistType=user";
                        }
                        WSHandler.getRequest(getMainActivity(), groupAddURL, new WSHandler.GreekResponseCallback() {
                            ...
                            public void onSuccess(JSONObject response) {
                                try {
                                    hideProgress();
                                    GreekDialog.alertDialog(getMainActivity(), 0, getString(GREEK), getString(R.string.GR_ADDED_SUCCESS_MSG), "OK", false, new DialogListener() {
                        ...
                        GreekDialog.alertDialog(getMainActivity(), 0, getString(GREEK), getString(R.string.GR_GROUP_NAME_EXIST_MSG), "OK", true, new DialogListener() {
                    ...
                    GreekDialog.alertDialog(getMainActivity(), 0, getString(GREEK), getString(R.string.GR_GROUP_NAME_ERROR_MSG), "OK", true, new DialogListener() {
    ...
    private void setupViews() {
        ...
        if (getArguments().getString("groupCount") == null) {
            ...
            symbolName.setSelection(symbolName.getText().length());
        }
        else {
