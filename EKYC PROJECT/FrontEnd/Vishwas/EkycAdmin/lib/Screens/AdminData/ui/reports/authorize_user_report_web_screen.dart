...
class AuthorizeUserReportWebScreen extends StatefulWidget {
  final String title;
  const AuthorizeUserReportWebScreen({
    Key? key,
    required this.title,
  }) : super(key: key);

  @override
  State<AuthorizeUserReportWebScreen> createState() => _AuthorizeUserReportWebScreenState();
}

class _AuthorizeUserReportWebScreenState extends State<AuthorizeUserReportWebScreen> {
                            ...
                            children: [
                              InkWell(
                                ...
                                onTap: () async {
                                  ...
                                  document.dispose();
                                  await helper.FileSaveHelper.saveAndLaunchFile(bytes, 'AuthorizeReport.xlsx');
                              ...
                              InkWell(
                                ...
                                onTap: () async {
                                  ...
                                  document.dispose();
                                  await helper.FileSaveHelper.saveAndLaunchFile(bytes, 'AuthorizeReport.pdf');
                                },
      ...
      child: FutureBuilder<List<CommonDataGridTableResponseModel>>(
        ...
        builder: (context, snapshot) {
          if ((snapshot.data != null) && (snapshot.data!.isNotEmpty)) {
            final columnList = snapshot.data?.first.toJsonForAuthorizeUserReport().keys.toList() ?? [];
            if (columnWidth.isEmpty) {
              columnWidth = columnList.map((e) => Tuple2(e, double.nan)).toList();
            }
            final dataList = snapshot.data?.map((e) => e.toJsonForAuthorizeUserReport()).toList() ?? [];
          ...
          } else if (snapshot.hasError) {
            {
              ...
              final columnList = errorData[0].toJsonForAuthorizeUserReport().keys.toList();
              if (columnWidth.isEmpty) {
                columnWidth = columnList.map((e) => Tuple2(e, double.nan)).toList();
              }
              _dataGridSource = CommonDataGridSource(
