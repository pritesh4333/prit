package in.co.vyaparienterprise.ui.activity.invoice.SalesOrder;
...
public class CreateSalesDispatchActivity extends SohoActivity implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
...
//    TextView invoiceDeliveryHourMinuteTV;
    ...
    TextView orderdate;
    @BindView(R.id.createinvoicedocumentdate)
    TextView createinvoicedocumentdate;
    ...
    private KeyValue keyvaluePOS;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_create_invoice);
        setToolbarConfig(getString(R.string.sales_dispacth), false);
        ButterKnife.bind(this);
        mContext = this;
        ordernumber.setText(getString(R.string.dispatchNumber));
        orderdate.setText(getString(R.string.dispatchDate));
        invoice_amount.setText(getString(R.string.Dispatch)+" "+getString(R.string.amount));
...
//        }
    ...
    @OnClick(R.id.create_invoice_documentdate)
   ...
   /* @OnCheckedChanged(R.id.create_invoice_is_kdv_include)
    ...
    public void createInvoiceClick() {
        ...
        String content = String.format(getString(R.string.create_Sales_Dispatch_approve_message), invoice.getFirm().getName(), CurrencyUtil.doubleToCurrency(invoice.getTotalAmount(), invoice.getCurrency()), date);
        String title;
        if (isEditInvoice) {
            if (InvoicesAdapter.CopyInvoiceDetails.equalsIgnoreCase("")) {
                title = getString(R.string.edit_Sales_dispatch);
            } else {
                title = getString(R.string.create_Sales_dispatch);
            }
        } else {
            title = getString(R.string.create_Sales_dispatch);
        }
    ...
    private void createInvoice() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(CreateSalesDispatchActivity.this);
    ...
    private void createInvoiceCall() {
        ...
        setPOS(keyvaluePOS);
        if (invoice.getPOS()==null){
            Toasty.error(mContext, getResources().getString(R.string.select_pos)).show();
        }else {
            Utils.showLoading(mContext);
            invoice.setDescription(discription.getText().toString());
            invoice.setNote(Note.getText().toString());
            invoice.setDispatchType(1);
            ...
            InvoiceService.addDispatch(invoice, new ServiceCall<BaseModel<InvoiceDTO>>() {
                @Override
    ...
    private void updateInvoiceCall() {
        ...
        if (invoice.getPOS()==null){
            ...
            InvoiceService.updateDispatch(invoice, new ServiceCall<BaseModel<InvoiceDTO>>() {
                @Override
                public void onResponse(boolean isOnline, BaseModel<InvoiceDTO> response) {
                    Utils.hideLoading();
                    if (!response.isError()) {
    ...
    private void deleteInvoiceCall() {
        ...
        InvoiceService.deleteDispatch(invoice.getId(), 1, new ServiceCall<BaseModel<InvoiceDTO>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<InvoiceDTO> response) {
                Utils.hideLoading();
                if (!response.isError()) {
    ...
    private void setDocumentDate() {
        if (InvoicesAdapter.CopyInvoiceDetails.equalsIgnoreCase("")) {
            ...
            createinvoicedocumentdate.setText(invoiceDayMonth);
        }else{
            Date dates = new Date(new Date().getTime());
            CalendarModel cm = DateUtil.dateToCM(dates);
            SimpleDateFormat sdfs = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
                ...
                Utils.generateDialog(mContext, invoiceLine.getProduct().getName(), null,
                        getString(R.string.edit_product), new MaterialDialog.SingleButtonCallback() {
                            ...
                            public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
                                ...
                                bundle.putString("type","SalesDispatch");
    ...
    private void setAddressSelectArea() {
        ...
        if (address != null && address.getDescription() != null) {
            ...
            deliveryAddressTV.setText(address.getAddressText());
        }
    }
    ...
    private void setPOS(KeyValue keyvalue) {
        invoice.setPOS(keyvalue);
    }
    ...
    private void getPlaseOfSale() {
            ...
            CommonService.getInvoicePos(new ServiceCall<BaseModel<List<POS>>>() {
                ...
                public void onResponse(boolean isOnline, BaseModel<List<POS>> response) {
                    ...
                    if (isEditInvoice) {
                        try {
                            String firmstate = invoice.getPOS().getValue();
                            for (int i = 0; i < Listpos.size(); i++) {
                                if (firmstate.equalsIgnoreCase(Listpos.get(i).getStateCode())) {
                                    invoice_pos.setSelection(i);
                                    keyvaluePOS= new KeyValue(Listpos.get(i).getCode(),Listpos.get(i).getStateCode(),Listpos.get(i).getLogicalReference());
                                }
                            }
                        }catch(Exception e){
                        ...
                        invoice_pos.setOnItemSelectedListener(new MyItemSelectedListener() {
                            ...
                            public void onItemSelected(int position) {
                                ...
                                keyvaluePOS= new KeyValue(Listpos.get(position).getCode(),Listpos.get(position).getStateCode(),Listpos.get(position).getLogicalReference());
                            }
                        });
                    }
                }
    ...
    private void getInvoiceNumber() {
        if (isEditInvoice) {
            if (InvoicesAdapter.CopyInvoiceDetails.equalsIgnoreCase("")) {
                ...
                CommonService.getDispatchInvoiceNumber(invoice.getInvoiceType(), new ServiceCall<BaseModel<String>>() {
                    @Override
                    public void onResponse(boolean isOnline, BaseModel<String> response) {
    ...
    private void updateInvoice() {
        ...
        Utils.showLoading(mContext);
        MobileConstants.invoiceId = invoice.getId();
        InvoiceService.calculateDispatch(invoice, new ServiceCall<BaseModel<InvoiceDTO>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<InvoiceDTO> response) {
                Utils.hideLoading();
                if (!response.isError()) {
        ...
        if (requestCode == Constants.SELECT_FIRM_FOR_INVOICE) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (firm != null) {
                    ...
                    String firmstate=firm.getState().getKey();
                    for (int i=0;i<Listpos.size();i++){
...
protected  void setDiscriptionNotes(){
    ...
    protected void displayShowcaseSelectProduct() {
        if (!isEditInvoice) {
            MainActivity.Type="SalesDispatch";
