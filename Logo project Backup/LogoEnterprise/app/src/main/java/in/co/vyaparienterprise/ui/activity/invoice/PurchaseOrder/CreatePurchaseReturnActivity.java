...
public class CreatePurchaseReturnActivity extends SohoActivity implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
...
//    TextView invoiceDeliveryHourMinuteTV;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setToolbarConfig(getString(R.string.purchases_return), false);
...
//        }
   ...
   /* @OnCheckedChanged(R.id.create_invoice_is_kdv_include)
    ...
    public void selectProductClick() {
        ...
        i.putExtra("type","PurchaseReturn");
    ...
    public void createInvoiceClick() {
        ...
        String content = String.format(getString(R.string.create_Purchase_Return_approve_message), invoice.getFirm().getName(), CurrencyUtil.doubleToCurrency(invoice.getTotalAmount(), invoice.getCurrency()), date);
        String title;
        if (isEditInvoice) {
            if (InvoicesAdapter.CopyInvoiceDetails.equalsIgnoreCase("")) {
                title = getString(R.string.edit_Purchase_return);
            }else{
                title = getString(R.string.create_Purchase_return);
            }
        } else {
            title = getString(R.string.create_Purchase_return);
        }
    ...
    private void createInvoice() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(CreatePurchaseReturnActivity.this);
    ...
    private void deleteInvoiceCall() {
        ...
        InvoiceService.deleteInvoice(invoice.getId(), 5, new ServiceCall<BaseModel<InvoiceDTO>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<InvoiceDTO> response) {
                Utils.hideLoading();
                if (!response.isError()) {
                ...
                Utils.generateDialog(mContext, invoiceLine.getProduct().getName(), null,
                        getString(R.string.edit_product), new MaterialDialog.SingleButtonCallback() {
                            ...
                            public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
                                ...
                                bundle.putString("type","PurchaseReturn");
...
protected  void setDiscriptionNotes(){
    ...
    protected void displayShowcaseSelectProduct() {
        if (!isEditInvoice) {
            MainActivity.Type="PurchaseReturn";
