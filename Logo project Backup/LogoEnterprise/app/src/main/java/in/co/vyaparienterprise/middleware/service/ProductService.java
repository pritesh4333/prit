...
public class ProductService {
    ...
    public static synchronized void productSlip(int i, ProductSlip productslip, ServiceCall<BaseModel<String>> serviceCall) {
        serviceCall.start();
        productSlipCall( i,productslip, serviceCall);
    }

    private static synchronized void productSlipCall(int i, ProductSlip productslip, final ServiceCall serviceCall) {

        Gson gson = new Gson();
        String jsn= gson.toJson(productslip);
        Log.e("JSON OBJECT-",jsn);
        Call call = VyapariApp.getApiService().createproductSlip(i,productslip);
        new ServiceManager<BaseModel<String>>().ServiceCall(call, new ServiceCall<BaseModel<String>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<String> response) {
                serviceCall.onResponse(true, response);
