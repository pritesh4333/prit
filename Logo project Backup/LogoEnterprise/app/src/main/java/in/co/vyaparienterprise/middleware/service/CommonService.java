...
import in.co.vyaparienterprise.BuildConfig;
...
import in.co.vyaparienterprise.model.VersionControl;
import in.co.vyaparienterprise.model.request.Crash;
...
import in.co.vyaparienterprise.model.response.dto.GstinForVyapariDTO;
import in.co.vyaparienterprise.model.response.dto.SectorDTO;
...
public class CommonService {
    ...
    public static synchronized void DashbordInformation(ServiceCall<BaseModel<ArrayList<KeyValue>>> serviceCall) {
        serviceCall.start();
        DashbordInformationCall( serviceCall);
    }

    private static void DashbordInformationCall( final ServiceCall<BaseModel<ArrayList<KeyValue>>> serviceCall) {
        Call<BaseModel<ArrayList<KeyValue>>> call = VyapariApp.getApiService().DashbordInformation();
        new ServiceManager<BaseModel<ArrayList<KeyValue>>>().ServiceCall(call, new ServiceCall<BaseModel<ArrayList<KeyValue>>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<ArrayList<KeyValue>> response) {
                serviceCall.onResponse(true, response);
    ...
    public static synchronized void DashbordInformationTopItemsGraph(String queryBase, ServiceCall<BaseModel<ArrayList<KeyValue>>> serviceCall) {
        serviceCall.start();
        DashbordInformationGraphTopItemsCall(queryBase, serviceCall);
    }

    private static void DashbordInformationGraphTopItemsCall(String queryBase, final ServiceCall<BaseModel<ArrayList<KeyValue>>> serviceCall) {
        Call<BaseModel<ArrayList<KeyValue>>> call = VyapariApp.getApiService().TopItems(queryBase);
        new ServiceManager<BaseModel<ArrayList<KeyValue>>>().ServiceCall(call, new ServiceCall<BaseModel<ArrayList<KeyValue>>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<ArrayList<KeyValue>> response) {
                serviceCall.onResponse(true, response);
    ...
    public static synchronized void DashbordInformationTopCustomerGraph(ServiceCall<BaseModel<ArrayList<KeyValue>>> serviceCall) {
        serviceCall.start();
        DashbordInformationGraphTopCustomerCall( serviceCall);
    }

    private static void DashbordInformationGraphTopCustomerCall( final ServiceCall<BaseModel<ArrayList<KeyValue>>> serviceCall) {
        Call<BaseModel<ArrayList<KeyValue>>> call = VyapariApp.getApiService().TopCustomers();
        new ServiceManager<BaseModel<ArrayList<KeyValue>>>().ServiceCall(call, new ServiceCall<BaseModel<ArrayList<KeyValue>>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<ArrayList<KeyValue>> response) {
                serviceCall.onResponse(true, response);
    ...
    public static synchronized void getInvoicePos(ServiceCall<BaseModel<List<POS>>> serviceCall) {
        serviceCall.start();
        getInvoicePosCall(serviceCall);
    }

    private static void getInvoicePosCall(final ServiceCall<BaseModel<List<POS>>> serviceCall) {
        Call<BaseModel<List<POS>>> call = VyapariApp.getApiService().getInvoicePOS();
        new ServiceManager<BaseModel<List<POS>>>().ServiceCall(call, new ServiceCall<BaseModel<List<POS>>>() {
            @Override
    ...
    //Order Invoice
    public static synchronized void getOrderInvoiceNumber(int type, ServiceCall<BaseModel<String>> serviceCall) {
        serviceCall.start();
        getOrderInvoiceNumberCall(type, serviceCall);
    }

    private static synchronized void getOrderInvoiceNumberCall(int type, final ServiceCall<BaseModel<String>> serviceCall) {
        Call<BaseModel<String>> call = null;
        switch (type) {
            case Constants.SALES_INVOICE:
                call = VyapariApp.getApiService().getOrderinvoiceNumber(1);
                break;
            case Constants.PURCHASE_INVOICE:
                call = VyapariApp.getApiService().getOrderinvoiceNumber(2);
                break;
        }
        if (call == null) {
            serviceCall.onFailure(true, null);
    ...
    //Order Invoice
    public static synchronized void getDispatchInvoiceNumber(int type, ServiceCall<BaseModel<String>> serviceCall) {
        serviceCall.start();
        getDispatchInvoiceNumberCall(type, serviceCall);
    }

    private static synchronized void getDispatchInvoiceNumberCall(int type, final ServiceCall<BaseModel<String>> serviceCall) {
        Call<BaseModel<String>> call = null;
        switch (type) {
            case Constants.SALES_INVOICE:
                call = VyapariApp.getApiService().getDispatchinvoiceNumber(1);
                break;
            case Constants.PURCHASE_INVOICE:
                call = VyapariApp.getApiService().getDispatchinvoiceNumber(2);
                break;
        }
        if (call == null) {
            serviceCall.onFailure(true, null);
