...
import in.co.vyaparienterprise.ui.adapter.ScapAdapter;
...
public class ScrapActivity extends SohoActivity implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private ScapAdapter productsAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setToolbarConfig(getString(R.string.scrap), false);
...
//
//
////                                productlistdublicatecode.add(productSumList.get(i).getCode());
////                                productlistdublicateInventory.add(productSumList.get(i).getActualInventory());
//                            }
                            try {
    ...
    @OnClick(R.id.scrap_inventory)
    public void savescrapInventoryclick() {

        ArrayList<ProductInventory> scrapchangelist = new ArrayList<ProductInventory>();
        ...
        for (int ii = 0; ii < productlistEdit.size(); ii++) {
            ...
            for (int iik = 0; iik < productSumListOrignal.size(); iik++) {

                if (productSumListOrignal.get(iik).getCode().equalsIgnoreCase(productlistEdit.get(ii).getCode())){
                double dublicate = productSumListOrignal.get(iik).getActualInventory();
                double dublicate1 = productlistEdit.get(ii).getActualInventory();
                    ...
                    if (dublicate != dublicate1) {
                        ...
                        prodinventory.setProductCode(productSumListOrignal.get(iik).getCode());
                        prodinventory.setProductDesc(productSumListOrignal.get(iik).getName());
                        prodinventory.setQty(dublicate1);
                        prodinventory.setUnit(Integer.valueOf(productSumListOrignal.get(iik).getMainUnitReference()));
                        prodinventory.setInventory(dublicate1);
                        scrapchangelist.add(prodinventory);
        ...
        createProductSlipforScrap(3, scrapchangelist);


    }
    private void createProductSlipforScrap(int i, ArrayList<ProductInventory> productdiff) {
        Utils.showLoading(ScrapActivity.this);
    ...
    private void resetList() {
        productSumList = new ArrayList<>();
        productsAdapter = new ScapAdapter(productSumList);
