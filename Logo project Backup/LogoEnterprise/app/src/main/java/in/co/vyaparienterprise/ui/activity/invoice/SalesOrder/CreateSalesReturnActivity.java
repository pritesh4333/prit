...
public class CreateSalesReturnActivity extends SohoActivity implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
...
//    TextView invoiceDeliveryHourMinuteTV;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setToolbarConfig(getString(R.string.sales_return), false);
        ButterKnife.bind(this);
        mContext = this;
        invoice_amount.setText(getString(R.string.Return)+" "+getString(R.string.amount));
        ordernumber.setText(getString(R.string.ReturnNumber));
        orderdate.setText(getString(R.string.ReturnDate));
        documentnumber.setText(getString(R.string.documentnumber));
...
//        }
   ...
   /* @OnCheckedChanged(R.id.create_invoice_is_kdv_include)
    ...
    public void createInvoiceClick() {
        ...
        String content = String.format(getString(R.string.create_Sales_Return_approve_message), invoice.getFirm().getName(), CurrencyUtil.doubleToCurrency(invoice.getTotalAmount(), invoice.getCurrency()), date);
        String title;
        if (isEditInvoice) {
            if (InvoicesAdapter.CopyInvoiceDetails.equalsIgnoreCase("")) {
                title = getString(R.string.edit_Sales_return);
            }else{
                title = getString(R.string.create_Sales_return);
            }
        } else {
            title = getString(R.string.create_Sales_return);
        }
    ...
    private void createInvoice() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(CreateSalesReturnActivity.this);
    ...
    private void deleteInvoiceCall() {
        ...
        InvoiceService.deleteInvoice(invoice.getId(), 4, new ServiceCall<BaseModel<InvoiceDTO>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<InvoiceDTO> response) {
                Utils.hideLoading();
                if (!response.isError()) {
                ...
                Utils.generateDialog(mContext, invoiceLine.getProduct().getName(), null,
                        getString(R.string.edit_product), new MaterialDialog.SingleButtonCallback() {
                            ...
                            public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
                                ...
                                bundle.putString("type","SalesReturn");
