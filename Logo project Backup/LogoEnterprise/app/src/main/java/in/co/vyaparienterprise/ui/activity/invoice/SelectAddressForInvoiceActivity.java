...
import in.co.vyaparienterprise.ui.activity.common.address.CreateAddressActivity;
import in.co.vyaparienterprise.ui.adapter.invoice.InvoiceSelectFirmAddressAdapter;
...
public class SelectAddressForInvoiceActivity extends SohoActivity {
    ...
    public void fabClick() {
        ...
        Intent activity2Intent = new Intent(getApplicationContext(), CreateAddressActivity.class);
        Bundle bundle = new Bundle();
        bundle.putParcelable(Constants.FIRM, firm);
        activity2Intent.putExtras(bundle);
        startActivityForResult(activity2Intent, Constants.REFRESH_CODE);
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == Constants.REFRESH_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                firm=data.getParcelableExtra(Constants.FIRM);
                if (newAddress != null) {
                    if (action == Constants.ADD) {

                        addressList.remove(clickedAddress);
                        addressList.add( newAddress);
                        firm.getShippingAddresses().add(newAddress);
                    }

                    //firm.setState(newAddress.getState());
    ...
    private void updateFirmCall(final Firm firmRequest) {
            ...
            public void onResponse(boolean isOnline, BaseModel<FirmDTO> response) {
                ...
                if (!response.isError()) {
                    ...
                    if (firmDTO.getShippingAddresses() != null) {
                        ...
                        invoiceSelectFirmAddressAdapter = new InvoiceSelectFirmAddressAdapter(firmRequest.getShippingAddresses());
                        addressesRV.setAdapter(invoiceSelectFirmAddressAdapter);
                    }
                } else {
