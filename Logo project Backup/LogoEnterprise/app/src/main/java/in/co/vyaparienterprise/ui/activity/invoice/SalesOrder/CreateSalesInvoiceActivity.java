...
public class CreateSalesInvoiceActivity extends SohoActivity implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
...
//    TextView invoiceDeliveryHourMinuteTV;
    ...
    private String salestoinvoice="22";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = this;
        invoice_amount.setText(getString(R.string.invoice)+" "+getString(R.string.amount));
        ordernumber.setText(getString(R.string.invoiceNumber));
        orderdate.setText(getString(R.string.invoiceDate));
        invoice = DataUtil.getBundleAndRemove(Invoice.class);
        try {
              salestoinvoice = getIntent().getExtras().getString("SalesOrderToInvoice");
            invoice.setOrderNumber(invoice.getInvoiceNumber());
            Log.e("SalesOrderToInvoice", salestoinvoice);
        }catch(Exception e){
...
//        }
   ...
   /* @OnCheckedChanged(R.id.create_invoice_is_kdv_include)
    ...
    public void createInvoiceClick() {
        ...
        String content = String.format(getString(R.string.create_Sales_invoice_approve_message), invoice.getFirm().getName(), CurrencyUtil.doubleToCurrency(invoice.getTotalAmount(), invoice.getCurrency()), date);
        String title;
        if (isEditInvoice) {
            if (InvoicesAdapter.CopyInvoiceDetails.equalsIgnoreCase("")) {
                title = getString(R.string.edit_Sales_invoice);
            } else {
                title = getString(R.string.create_Sales_invoice);
            }
        } else {
            title = getString(R.string.create_Sales_invoice);
        }
    ...
    private void createInvoice() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(CreateSalesInvoiceActivity.this);
    ...
    private void createInvoiceCall() {
        ...
        if (salestoinvoice.equalsIgnoreCase("SalesOrderToInvoice")){
            InvoiceService.OrdertoInvoicebilling(invoice, new ServiceCall<BaseModel<InvoiceDTO>>() {
                @Override
    ...
    private void deleteInvoiceCall() {
        ...
        InvoiceService.deleteInvoice(invoice.getId(),1, new ServiceCall<BaseModel<InvoiceDTO>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<InvoiceDTO> response) {
                Utils.hideLoading();
                if (!response.isError()) {
                ...
                Utils.generateDialog(mContext, invoiceLine.getProduct().getName(), null,
                        getString(R.string.edit_product), new MaterialDialog.SingleButtonCallback() {
                            ...
                            public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
                                ...
                                bundle.putString("type","SalesInvoice");
...
protected  void setDiscriptionNotes(){
    ...
    protected void displayShowcaseSelectProduct() {
        if (!isEditInvoice) {
            MainActivity.Type="SalesInvoice";
