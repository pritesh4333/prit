...
public class CreatePurchaseReceiptActivity extends SohoActivity implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
...
//    TextView invoiceDeliveryHourMinuteTV;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setToolbarConfig(getString(R.string.purchases_receipt), false);
        ButterKnife.bind(this);
        mContext = this;
        login= (LoginDTO) Utils.getObjectSharedPreferencesValue(mContext, Constants.LOGIN_RESPONSE_INFO, LoginDTO.class);
        ordernumber.setText(getString(R.string.ReceiptNumber));
        orderdate.setText(getString(R.string.ReceiptDate));
        invoice_amount.setText(getString(R.string.Receipt)+" "+getString(R.string.amount));
...
//        }
   ...
   /* @OnCheckedChanged(R.id.create_invoice_is_kdv_include)
    ...
    public void selectProductClick() {
        ...
        i.putExtra("type","PurchaseReceipt");
    ...
    public void createInvoiceClick() {
        ...
        String content = String.format(getString(R.string.create_Purchase_Receipt_approve_message), invoice.getFirm().getName(), CurrencyUtil.doubleToCurrency(invoice.getTotalAmount(), invoice.getCurrency()), date);
        String title;
        if (isEditInvoice) {
            if (InvoicesAdapter.CopyInvoiceDetails.equalsIgnoreCase("")) {
                title = getString(R.string.edit_Purchase_receipt);
            }else{
                title = getString(R.string.create_Purchase_receipt);
            }
        } else {
            title = getString(R.string.create_Purchase_receipt);
        }
    ...
    private void createInvoice() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(CreatePurchaseReceiptActivity.this);
    ...
    private void deleteInvoiceCall() {
        ...
        InvoiceService.deleteDispatch(invoice.getId(), 2, new ServiceCall<BaseModel<InvoiceDTO>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<InvoiceDTO> response) {
                Utils.hideLoading();
                if (!response.isError()) {
                ...
                Utils.generateDialog(mContext, invoiceLine.getProduct().getName(), null,
                        getString(R.string.edit_product), new MaterialDialog.SingleButtonCallback() {
                            ...
                            public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
                                ...
                                bundle.putString("type","PurchaseReceipt");
    ...
    private void getPlaseOfSale() {
            ...
            CommonService.getInvoicePos(new ServiceCall<BaseModel<List<POS>>>() {
                ...
                public void onResponse(boolean isOnline, BaseModel<List<POS>> response) {
                    ...
                    if (isEditInvoice) {
                        try{
                        String firmstates=invoice.getPOS().getValue();
                        for (int i=0;i<Listpos.size();i++){
                            if (firmstates.equalsIgnoreCase(Listpos.get(i).getDescription())){
                                invoice_pos.setSelection(i);
                            }
                        }
                        }catch(Exception e){
...
protected  void setDiscriptionNotes(){
    ...
    protected void displayShowcaseSelectProduct() {
        if (!isEditInvoice) {
            MainActivity.Type="PurchaseReceipt";
