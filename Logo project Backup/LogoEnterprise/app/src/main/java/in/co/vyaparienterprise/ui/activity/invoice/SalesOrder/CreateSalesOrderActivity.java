...
public class CreateSalesOrderActivity extends SohoActivity implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
...
//    TextView invoiceDeliveryHourMinuteTV;
    ...
    @BindView(R.id.create_invoice_showDatePickerDialogDeliveryDate)
    LinearLayout create_invoice_showDatePickerDialogDeliveryDate;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        create_invoice_showDatePickerDialogDeliveryDate.setVisibility(View.VISIBLE);
...
//    }
   ...
   /* @OnCheckedChanged(R.id.create_invoice_is_kdv_include)
    ...
    public void createInvoiceClick() {
        ...
        String content = String.format(getString(R.string.create_Sales_Order_approve_message), invoice.getFirm().getName(), CurrencyUtil.doubleToCurrency(invoice.getTotalAmount(), invoice.getCurrency()), date);
        String title;
        if (isEditInvoice) {
            if (InvoicesAdapter.CopyInvoiceDetails.equalsIgnoreCase("")) {
                title = getString(R.string.edit_Sales_order);
            } else {
                title = getString(R.string.create_Sales_order);
            }
        } else {
            title = getString(R.string.create_Sales_order);
        }
    ...
    private void createInvoice() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(CreateSalesOrderActivity.this);
    ...
    private void deleteInvoiceCall() {
        ...
        InvoiceService.deleteOrder(invoice.getId(), 1, new ServiceCall<BaseModel<InvoiceDTO>>() {
            @Override
            public void onResponse(boolean isOnline, BaseModel<InvoiceDTO> response) {
                Utils.hideLoading();
                if (!response.isError()) {
                ...
                Utils.generateDialog(mContext, invoiceLine.getProduct().getName(), null,
                        getString(R.string.edit_product), new MaterialDialog.SingleButtonCallback() {
                            ...
                            public void onClick(MaterialDialog dialog, DialogAction which) {
                                ...
                                bundle.putString("type", "SalesOrder");
