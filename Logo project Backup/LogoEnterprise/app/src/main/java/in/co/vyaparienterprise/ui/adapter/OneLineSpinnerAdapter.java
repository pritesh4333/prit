...
public class OneLineSpinnerAdapter extends BaseAdapter {
    ...
    public OneLineSpinnerAdapter(Context applicationContext, ArrayList<String> listofposdecription, String s, String s1) {
        this.context = applicationContext;
        if (strings == null) {
            strings = new ArrayList<>();
        }
        ...
        this.strings = new ArrayList<>();
        for (String collectiondto : listofposdecription) {
            this.strings.add(new KeyValue(collectiondto,""));
        }
        this.color = Color.parseColor("#FFFFFF");
        inflter = (LayoutInflater.from(applicationContext));
    }

    public OneLineSpinnerAdapter(Context applicationContext, List<POS> listpos, int whiteColor,int color) {
        this.context = applicationContext;
        if (strings == null) {
            strings = new ArrayList<>();
        }
        this.strings = new ArrayList<>();
        for (POS pos : listpos) {
            this.strings.add(new KeyValue(pos.getCode(), pos.getDescription()));
        }
        this.color = whiteColor;
        inflter = (LayoutInflater.from(applicationContext));
    }

    public OneLineSpinnerAdapter(Context applicationContext, ArrayList<String> formtypList, String s, String s1, String s2) {
        this.context = applicationContext;
        if (strings == null) {
            strings = new ArrayList<>();
        }
        ...
        this.strings = new ArrayList<>();
        for (String collectiondto : formtypList) {
            this.strings.add(new KeyValue(collectiondto,collectiondto));
        }
